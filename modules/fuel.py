...
def cargar_datos_gasolina():
    ...
    if os.path.exists(csv_path):
        try:
            ...
            pass
    return pd.DataFrame({'estacion': ['REPSOL Aranjuez', 'CEPSA Aranjuez', 'Shell Aranjuez', 'REPSOL Seseña', 'BP Seseña', 'REPSOL Seseña Premium'], 'precio': [1.759, 1.779, 1.789, 1.749, 1.799, 1.769], 'localidad': ['Aranjuez', 'Aranjuez', 'Aranjuez', 'Seseña', 'Seseña', 'Seseña'], 'lat': [40.0, 40.01, 40.02, 40.1, 40.11, 40.12], 'lon': [-3.6, -3.61, -3.62, -3.5, -3.51, -3.52]})
...
def analizar_gasolina(df, presupuesto=100):
    ...
    return {'mejor': {'estacion': min_row.get('estacion', 'No disponible'), 'precio': float(min_row['precio']), 'localidad': min_row.get('localidad', 'No disponible')}, 'peor': {'estacion': max_row.get('estacion', 'No disponible'), 'precio': float(max_row['precio']), 'localidad': max_row.get('localidad', 'No disponible')}, 'promedio': round(avg_price, 3), 'presupuesto': {'total': presupuesto_dinero, 'litros_posibles': litros_posibles, 'porcentaje_gastado': porcentaje_gastado, 'alerta': alerta_presupuesto}, 'estaciones': [{'nombre': row.get('estacion', 'No disponible'), 'precio': float(row['precio']), 'localidad': row.get('localidad', 'No disponible'), 'lat': float(row.get('lat', 40.0)), 'lon': float(row.get('lon', -3.6))} for _, row in df_clean.iterrows()]}
...
def calcular_comparador(gasolina_data):
    ...
    return {'seseña': mejor_seseña, 'aranjuez': mejor_aranjuez, 'diferencia_precio': diferencia, 'ahorro_deposito': ahorro_deposito, 'ahorro_mensual_estimado': ahorro_mensual, 'recomendacion': recomendacion}
